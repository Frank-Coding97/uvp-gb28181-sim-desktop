...
跨模块公共设施:统一错误、领域标识、传输类型、日志初始化、时间/校时(`clock`)。所有其它 crate 复用。

`clock` 模块:UNIX 纪元 ↔ ISO8601(自实现民用历法换算,无第三方依赖),网络校时偏移(解析平台 REGISTER 200 的 `Date` 头,记录偏移,`synced_iso8601()` 生成对齐平台的时间戳)。
