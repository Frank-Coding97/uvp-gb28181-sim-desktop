...
| **M6 协议全覆盖对齐上游** | 扩展查询/控制/抓拍/升级/主动通知/信令健壮性 + H.265/AAC + 多通道·OSD·语音广播 | FR-17~19, FR-30~36 | 对标 uvp-gb28181-sim GB-2022 覆盖矩阵 100%(去除明确不做项);SIP 注入/真机 WVP 验证 | ✅ 7 阶段完成(单测/渲染核对;真机 WVP 待复验) |

### M6 分档推进(每档:文档→代码→单测/注入验证→更新覆盖矩阵→commit)

| 档 | 内容 | 需求 | 状态 |
|---|---|---|---|
| P1 查询 | AlarmStatus / HomePositionQuery / StorageCardStatusQuery / CruiseTrackListQuery / CruiseTrackQuery / PTZPreciseStatusQuery / MobilePosition 单次 / ConfigDownload+VideoParamOpt | FR-17 | ✅ 单测(SIP 注入/真机待复验) |
| P2 控制 | PTZPreciseCtrl / 巡航控制 / 辅助控制 / Focus / TargetTrack / FormatSDCard | FR-18 | ✅ 单测(真机待复验) |
| P3 抓拍升级 | SnapShotCmd / SnapShotConfig(HTTP PUT) / DeviceUpgrade 4 步 | FR-19, FR-30 | ✅ 单测(本地 HTTP 收图路径已测) |
| P4 通知健壮 | MediaStatus / Expires 续约 / RTCP SR / Catalog 增量 NOTIFY | FR-31, FR-32 | ✅ 单测 |
| P5 媒体 | H.265 / AAC | FR-33 | ✅ PSM 单测(音轨抽取/真机推流待 ffmpeg) |
| P6 多通道+OSD | 虚拟通道 CRUD+模板 / 报警通道节点 / OSD 叠加 | FR-34, FR-35 | ✅ 单测 + headless Chrome 渲染;OSD 配置面对齐 |
| P7 语音广播 | Broadcast 应答 + 反向 INVITE UAC + 音频接收 | FR-36 | 🟡 握手+SDP offer 单测;反向 INVITE/收流本环境无法触发未实装 |
