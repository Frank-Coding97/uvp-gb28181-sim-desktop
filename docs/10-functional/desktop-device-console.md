...
| 设备配置 | device_id、密码(眼睛图标可切换明文/掩码)、通道名、视频源文件(可空;注册成功后启动单路采集供本地预览，平台点播复用同一批编码帧;支持 H.264/H.265 裸流及 MP4/FLV/MKV/MOV 容器,发布包内置 FFmpeg) |
...
- 配置视频源后,注册成功 → 本地预览开始出画面；平台点播 → 状态灯变「推流中」→ 平台复用同一批编码帧出画面；BYE 后平台推流停止而本地预览继续，注销后释放采集源。
